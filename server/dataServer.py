...
example program:
	...
	### declare runtime variables required by the receive system	
	packet_size = 512
	transReadyTimeout = 1100 # us
	trigDelay = 150 # us
	recLen = 2048
	
	### connect to the receive system and issue the resetVars command to initialize everything to defualt values
	d.connect()
	d.resetVars()
	
	### issue runtime variables to the receive system
	...
	### put the SoCs in a state to be ready to acquire data
	d.toggleDataAcq(1)
	
	### fire the array and acquire data
	for chargetimeN in range(0,nChargeTimes):
		for pulseN in range(0,nPulsesPerLoc):
			for locN in range(0,nSteeringLocations):
				...
				### tell the cServer the index location in the array to store the upcoming data 
				d.declareDataAcqIdx(chargetimeN,pulseN,locN)
				
				#---> update the steering location or other variables using the 'a'/'b' commands <---#
				...
				### check whether the data acquisition took less time than desired by the PRF and if so wait the appopriate amount of time before pulsing again 
				dt = time.time()-t0
				if( dt < 1.0/PRF ):
...
class dataServer():
	...
	def setSocTransReadyTimeout(self,to):
		# sets how often the SoCs check whether they have data ready to transmit. takes integer 'to' as input, units = us. lower values make the SoC check more often but burns cpu time by busy waiting. should be set as high as possible without slowing down the program.
	...
	def setDataArraySize(self,l1,l2,l3):
		# used to define the amount of data that will be collected during the experiment. takes integer values 'l1','l2', and 'l3' as input, unitless. memory must be allocated in the cServer before data acquisition begins. data is stored in a 5D array of size [l1,l2,l3,2*recLen,boardN], these values give the size of the first 3 dimensions of the data array in the cServer. 
	...
	def allocateDataArrayMemory(self):
		# tells the cServer to allocate the memory for the data to be acquired based on the inputs from the 'setRecLen' and 'setDataArraySize' commands
	...
	def setChannelMask(self,cmask,maskState):
		...
		3) pulse three - now in maskState 2, elements two through N are unmasked and elements zero and one are masked. upon triggering, the SoC will acquire data from elements two through N AND transmit the acquired data to the cServer. the acquired data will contain the signal from element zero from the first pulse, the signal from element one from the second pulse, and the signals from elements two through N from the third pulse. after transmitting the data, the maskState on the cServer and SoCs will be reset to 0.
		
		This lets you acquire data from multiple pulses without transfering it over ethernet to the cServer after each pulse, but it doesn't let you acquire multiple pulses from all elements simultaneously without masking others. That feature may or may not be built in in the future, but I'm not going to bother with it now because it would require a bigger rewrite of the code base than this solution did because of potential memory allocation/reallocation issues. It might also not be necessary if the slow bridge issue ever gets solved, but even if it doesn't the system is fast enough now to mostly get around it. It can offer potentially large speed ups though by throwing away all the data from one or the other GPIOs on the FPGA. Roughly 1/3 of the slow bridge time each is taken up by transferring data to the SoC from GPIO_0 and GPIO_1, so by setting the mask such that all channels on GPIO_0 or GPIO_1 are turned off, significant speed ups can be had. I haven't implemented a function to transfer subsets of data over ethernet yet though, so even if you do turn off half the channels, you still have to transmit a full sized array of data over ethernet to the cServer. Eventually I might add the ability to transfer subsets of data, but it'd require a bit of rewriting of the code base and hasn't been a significant enough bottle neck yet to do it.'''
